...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['c45_id'] = $rows['id_user']; // mengisi session
        $_SESSION['c45_nama'] = $rows['nama'];
        $_SESSION['c45_username'] = $rows['username'];
        $_SESSION['c45_level'] = $rows['level'];
        $_SESSION['c45_id_siswa'] = get_id_siswa_by_id_user($db, $rows['id_user']);

        $level_name = ($_SESSION['c45_level']==1)?"Bendahara":"Ketua";
        $_SESSION['c45_level_name'] = $level_name;
        $_SESSION['c45_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['c45_last_login'] = date("d-m-Y H:i:s");
